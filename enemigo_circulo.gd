...
# --- ARTE Y COLOR ---
var color_base = Color(0.9, 0.1, 0.3) # Rojo carmesí 
var color_sombra = Color(0.5, 0.0, 0.1)
...
func _ready():
	...
	add_to_group("elites") 
	add_to_group("enemigo") # Necesario para que las balas del jugador lo detecten
...
func _physics_process(delta):
	...
	if is_instance_valid(player): 
		...
		# Lógica de salto para perseguir al jugador implacablemente
	...
	# Polling para hacer daño (Seguro para No-Hit)
...
# --- INTELIGENCIA ARTIFICIAL: ATAQUE RADIAL ÉLITE ---
func _on_timer_ataque_timeout():
	...
	# AVISO (0.4s): El círculo palpita y brilla intensamente en carmesí
	tween.tween_property(self, "scale", Vector2(1.2, 1.2), 0.2).set_trans(Tween.TRANS_SINE)
	tween.parallel().tween_property(self, "modulate", Color(3.0, 0.5, 0.5), 0.4)
	tween.tween_property(self, "scale", Vector2(0.9, 0.9), 0.2).set_trans(Tween.TRANS_SINE)
	
	await tween.finished
	if not is_inside_tree(): return
	
	# ATAQUE: Pulso Radial de 8 direcciones simultáneas 
	...
	for i in range(cantidad_balas): 
		...
		bala.dano = 15 
		bala.velocidad = 350.0 # Proyectiles más rápidos por ser élite
...
func recibir_daño(cantidad):
	...
	if salud <= 0: 
		if ClassDB.class_exists("EfectoExplosion") or ResourceLoader.exists("res://efecto_explosion.gd"):
			...
			explosion.iniciar("circulo", color_base) 
