...
# --- ESTADÍSTICAS REBALANCEADAS (ÉLITE: ROMBO) ---
@export var velocidad = 115.0 
var velocidad_base: float
var velocidad_actual: float
@export var salud = 20 # Rebalanceado de 180 a 20 para la nueva escala
@export var dano_contacto = 15
@export var probabilidad_gema: float = 1.0
@export var xp_que_da: int = 8 # Da buena experiencia por ser Élite
...
# --- ARTE Y COLOR ---
var color_base = Color(0.1, 0.8, 0.7) # Cian / Verde agua
var color_sombra = Color(0.05, 0.4, 0.35)
...
# --- INTELIGENCIA ARTIFICIAL: ATAQUE ABANICO ---
func _on_timer_ataque_timeout():
	...
	# AVISO (0.4s): Se estira verticalmente (tensa el arco) y brilla en cian intenso
	tween.tween_property(self, "scale", Vector2(0.8, 1.3), 0.4).set_trans(Tween.TRANS_SINE)
	tween.parallel().tween_property(self, "modulate", Color(2.0, 3.0, 3.0), 0.4)
	
	await tween.finished
	if not is_inside_tree(): return
	
	# ATAQUE: Disparo en abanico (Centro, Arriba, Abajo)
	var dir_base = global_position.direction_to(player.global_position)
	var angulos = [dir_base.rotated(-0.4), dir_base, dir_base.rotated(0.4)]
	
	for d in angulos:
		var bala = proyectil_escena.instantiate()
		bala.dano = 10
		bala.velocidad = 350.0
		bala.scale = Vector2(0.8, 0.8) # Balas más pequeñas como indicaste
	...
	# RECUPERACIÓN: Vuelve a su forma original rápido
	var tween_rec = create_tween()
	tween_rec.tween_property(self, "scale", Vector2.ONE, 0.1)
...
func recibir_daño(cantidad):
	...
	if salud <= 0:
		if ClassDB.class_exists("EfectoExplosion") or ResourceLoader.exists("res://efecto_explosion.gd"):
			...
			explosion.iniciar("rombo", color_base) 
